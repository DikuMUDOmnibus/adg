...
sh_int                  gsn_llothbless; /* drow bless -Sage*/
...
//        load_gangs()
	load_disabled();
        ...
        load_hiscores();
	arena = FIGHT_CLEAR;
        //WRITE_STR(debug_last_command, "");
        //WRITE_STR(debug_last_character, "");
...
/* OLC
 * I left the Repop field here because I though that newbie programmers
like
...
/*
 * Snarf an 'area' header line.   Check this format.  MUCH better.  Add
fields
 * too.
 *
 * #AREAFILE
...
//	pMobIndex->damroll		= fread_number( fp );	/* Unused */
...
void load_rooms( FILE *fp )
{
	...
	if(IS_SET(pRoomIndex->room_flags,ROOM_TOTAL_DARKNESS))
		REMOVE_BIT(pRoomIndex->room_flags,ROOM_TOTAL_DARKNESS);
	    ...
	    if ( letter == 'D' )
            {
                ...
                pexit->vnum             = fread_number( fp );
                                /*   going to go to a proper flag loading system...
                switch ( locks )
                {
                ...
                case 2: pexit->exit_info = EX_ISDOOR | EX_PICKPROOF;
                        ...
                        break;
                case 3: pexit->exit_info = EX_ISDOOR;
                        pexit->rs_flags  = EX_ISDOOR;
                        break;
...
void area_update( void )
{
	 ...
	 * Check for PC's.
	 */
	if(IS_SET(pArea->bits,WASTE) && IS_SET(pArea->bits,UPDATEWASTE))
	{
		REMOVE_BIT(pArea->bits,WASTE);
		sta("The land around you shifts into something more recognizable.\n\r",pArea);
	}
	if(!IS_SET(pArea->bits,UPDATEWASTE))SET_BIT(pArea->bits,UPDATEWASTE);
	...
	if ( pArea->nplayer > 0 && pArea->age == 15 - 1 )
	{
		...
		{
		    send_to_char( "You hear an ancient bell toll in the distance.\n\r",pch );
		}
...
void reset_area( AREA_DATA *pArea )
{
	...
	case 'M':
	    ...
	    char_to_room( mob, pRoomIndex );
            if ( mob->in_room->area->mod > 0 )
            {
              mob->level += ( mob->in_room->area->mod * 25 );
              mob->max_hit += ( mob->in_room->area->mod * 2000 );
              mob->hit = mob->max_hit;
            }
            if ( mob->in_room->area->mod < 0 )
            {
...
//Commented out 1/15/00 by SaGe to fix crash bug.
/*
        if ( number_range( 1, 200 ) == 100 )
            {
 
                quest = create_object(get_obj_index(30054),0);
                free_string( quest->name );
                free_string( quest->short_descr );
                free_string( quest->description );
 
                quest->name = str_dup("treasure priceless quest gold");
                quest->short_descr = str_dup("A gold quest piece");
                quest->description = str_dup("A shiny gold coin lies here.");
                obj_to_char( quest, mob );
                SET_BIT(quest->quest,ITEM_EQUEST);
		if(mob->level < 15)
		{
		quest->cost = 15;
		}
                if(mob->level > 99)
                {
		quest->cost = 99 + (mob->level / 75);
		}
		else quest->cost = mob->level;
	...
	case 'E':
		...
		default:		olevel = 0;
break;
		case ITEM_PILL:		olevel = number_range(  0, 10 );
break;
		case ITEM_POTION:	olevel = number_range(  0, 10 );
break;
		case ITEM_SCROLL:	olevel = number_range(  5, 15 );
break;
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
   ...
   mob->hitroll	= (mob->level * 2.5);
    mob->damroll	= (mob->level * 2.5);

 if (mob->level>100) {mob->hitroll = mob->hitroll * 1.5;mob->damroll
*= 1.5;}

 if (mob->level>250) {mob->hitroll = mob->hitroll * .10;mob->damroll
*=.10;}
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    *obj		= obj_zero;
    ...
    else if (obj->pIndexData->vnum == 30039)
    {
        obj->condition = 100;
        obj->toughness = 100;
        obj->resistance = 1;
   ...
   /*Making Plasma Ball also a relic -Leon*/
    else if ( obj->pIndexData->vnum == 110)
    {
       SET_BIT(obj->quest, QUEST_RELIC);
    }
    else
 ...
 * Free a character.
 */
void free_char( CHAR_DATA *ch, bool quiet )
{
    OBJ_DATA *obj;
    OBJ_DATA *obj_next;
    AFFECT_DATA *paf;
...
*/
/*
void log_string_special( const char *str, const char *name) 
{
    char *strtime;
    char logout [MAX_STRING_LENGTH];
...
/*
 Changed & Added by Toaster 16/02/00 so some things can log to file
 without spamming the immortals.
*/
void log_string_quiet( const char *str )
{
    char *strtime;
    char logout [MAX_STRING_LENGTH];
...
/* Changed by Toaster to log player with log flag to player.log
void log_player(const char *str, CHAR_DATA *ch)
{
    FILE	*fp;
    ...
    sprintf( file_name, "%s%s", PLAYER_LOG_DIR, capitalize (ch->name) );
...
*/
void logf ( CHAR_DATA *ch, char *fmt, ... )
{
    log_string( fmt );
return;
}
