...
int get_curr_str( CHAR_DATA *ch )
{
    ...
    if (IS_CLASS(ch,CLASS_VAMPIRE) && IS_POLYAFF(ch, POLY_NEWZULO) )
    {
	return 40;
    }
...
/*
	...
	if (!duplicate) {
	  for ( objtmp = ch->carrying ; 
                objtmp != NULL ; 
                objtmp = objtmp->next_content ) { 
		
	    if (objtmp->wear_loc != WEAR_NONE) { //worn, check affects
	 			
	      for (tmp = objtmp->affected ; 
                   tmp != NULL ; 
                   tmp = tmp->next) {
	        if ( (tmp != paf) 
...
if ( !IS_NPC(ch) && (IS_SET(ch->newbits, NEW_DARKNESS)))
    ...
    else if ( !IS_NPC(ch) && IS_SET(ch->newbits, NEW_NEUTRAL))
    {
	if (ch->in_room != NULL)
	{
	    REMOVE_BIT(ch->in_room->room_flags, ROOM_SAFE); 
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
 ...
 if (ch->in_room != NULL && (ch->in_room->vnum != 2) &&
...
if (IS_SET(obj->spectype, SITEM_DRAGON) || obj->pIndexData->vnum == 29614)
...
if (IS_SET(obj->spectype, SITEM_DROW) || obj->pIndexData->vnum ==
	...
	return;
        ...
        if (IS_SET(obj->spectype, SITEM_MAGE) || obj->pIndexData->vnum == 29620 ||
        obj->pIndexData->vnum == 29622 || obj->pIndexData->vnum == 29621)
	...
	if (IS_SET(obj->spectype, SITEM_NINJA) || obj->pIndexData->vnum == 29687 ||
        obj->pIndexData->vnum == 29688)
...
CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch, CLASS_NINJA) &&  obj->chobj == NULL &&
 !IS_CLASS(ch, CLASS_DRAGON) && !IS_CLASS(ch, CLASS_DROW))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR
);
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM
...
/*
    ...
    if ((IS_NPC(ch) || (!IS_CLASS(ch, CLASS_MAGE))) &&
        IS_SET(obj->spectype, SITEM_MAGE))
    {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
        act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
        obj_from_char( obj );
 ...
 */
    if( IS_SET(obj->quest, QUEST_MASTERY) && IS_ITEMAFF(ch, ITEMA_MASTERY))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
  ...
  if (IS_SET(obj->spectype, SITEM_DEMONIC) && !IS_NPC(ch) && IS_DEMON(ch))
        {
	    if(ch->pcdata->stats[DEMON_POWER] < 17 )
	    {
		ch->pcdata->stats[DEMON_POWER]++;
                if (obj->specpower == ARM_BLACK) ch->pcdata->atm += 1;
                if (obj->specpower == ARM_RED   ) ch->pcdata->atm += 1;
                if (obj->specpower == ARM_COPPER ) ch->pcdata->atm += 2;
                if (obj->specpower == ARM_BRONZE ) ch->pcdata->atm += 2;
                if (obj->specpower == ARM_BRASS ) ch->pcdata->atm += 3;
	    }
        }
/*
     ...
     for( iWear = 0; iWear < MAX_WEAR; iWear++ )
         {
            ...
            {
                        if(obj->specpower == ARM_BLACK) ch->pcdata->atm  += 1;
                        if(obj->specpower == ARM_RED)  ch->pcdata->atm += 2;
                        if(obj->specpower == ARM_COPPER)  ch->pcdata->atm += 3;
                        if(obj->specpower == ARM_BRONZE)  ch->pcdata->atm += 4;
...
/* Item immunities - Tracker 03/18/03 */
        || IS_SET(obj->immune, IITEM_SLASH)
        || IS_SET(obj->immune, IITEM_STAB)
        || IS_SET(obj->immune, IITEM_SMASH)
        || IS_SET(obj->immune, IITEM_BEAST)
        || IS_SET(obj->immune, IITEM_GRAB)
        || IS_SET(obj->immune, IITEM_CHARM)
        || IS_SET(obj->immune, IITEM_HEAT)
        || IS_SET(obj->immune, IITEM_COLD)
        || IS_SET(obj->immune, IITEM_LIGHTNING
        || IS_SET(obj->immune, IITEM_ACID))
        || IS_SET(obj->immune, IITEM_SLEEP)
        || IS_SET(obj->immune, IITEM_DRAIN)
        || IS_SET(obj->immune, IITEM_HURL)
        || IS_SET(obj->immune, IITEM_BACKSTAB)
        || IS_SET(obj->immune, IITEM_KICK)
        || IS_SET(obj->immune, IITEM_DISARM)
        || IS_SET(obj->immune, IITEM_STEAL))
    {
	...
	if (IS_SET(obj->quest, QUEST_MASTERY))
	    SET_BIT(ch->itemaffect, ITEMA_MASTERY);
...
*/
	...
	if (IS_SET(obj->quest, QUEST_MASTERY))
	    REMOVE_BIT(ch->itemaffect, ITEMA_MASTERY);
	if (IS_SET(obj->quest, QUEST_ARTIFACT)) 
	     ...
	     && IS_SET(victim->act, PLR_WIZINVIS))
{
	...
	if (!IS_NPC(victim) && IS_HUNTER(victim) && IS_AFFECTED(victim, AFF_HIDE) ) {
	if (!IS_IMMORTAL(ch) && !IS_HUNTER(ch) )
	return FALSE;
	else return TRUE;
	}

	if (!IS_NPC(victim) && IS_NINJA(victim) && IS_AFFECTED(victim, AFF_HIDE) ) {
	if (!IS_IMMORTAL(ch) && !IS_NINJA(ch) )
	return FALSE;
	else return TRUE;
	}
...
(IS_SET(ch->newbits, NEW_DARKNESS))
    {
        if (!IS_IMMORTAL( ch ) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_VAMPIRE))
	    return FALSE;
        else
	    return TRUE;
    }

if ( IS_VAMPAFF(victim, VAM_OBFU) && !IS_IMMORTAL(ch) &&
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_SET(ch->garou1, GAROU_BLISSFUL) && IS_CLASS(ch, CLASS_WEREWOLF) )  
    return FALSE;
 ...
 if ( IS_SET ( ch->in_room->room_flags, ROOM_TOTAL_DARKNESS))
    {
       ...
       if (!IS_NPC(victim) && IS_HUNTER(victim) &&
...
char *item_type_name( OBJ_DATA *obj )
{
    ...
    case ITEM_WGATE:            return "rift";
    case ITEM_DTOKEN:           return "demon token";
    }
...
/*
    ...
    if ( vector & AFF_LLOTHBLESS      ) strcat( buf, " llothbless" );
*/
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_MENCHANT     ) strcat( buf, " mageenchant"  );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
*/
