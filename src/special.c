...
DECLARE_SPEC_FUN(	spec_clan_vampires	);
...
//    if ( !str_cmp( name, "spec_clan_torcalta"     ) ) return spec_clan_torcalta;
    if ( !str_cmp( name, "spec_clan_vampires"     ) ) return spec_clan_vampires;
    ...
    if ( !str_cmp( name, "spec_clan_landers"       ) ) return spec_clan_landers;
    ...
    if ( !str_cmp( name, "spec_decap"		  ) ) return spec_decap;
    return 0;
}
...
bool spec_cast_adept( CHAR_DATA *ch )
{
    ...
    {
    case 0:
	act( "$n utters the word 'egas'.", ch, NULL, NULL, TO_ROOM );
	spell_armor( skill_lookup( "armor" ), ch->level, ch, victim );
	return TRUE;

    case 1:
	act( "$n utters the word 'niator'.", ch, NULL, NULL, TO_ROOM );
	spell_bless( skill_lookup( "bless" ), ch->level, ch, victim );
	return TRUE;

    case 2:
	act( "$n utters the word 'rejit'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_blindness( skill_lookup( "cure blindness" ),
	    ch->level, ch, victim );
	return TRUE;

    case 3:
	act( "$n utters the word 'rivak'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_light( skill_lookup( "cure light" ),
	    ch->level, ch, victim );
	return TRUE;

    case 4:
	act( "$n utters the word 'rawalam'.", ch, NULL, NULL, TO_ROOM
);
...
/* Changed by Toaster 06/02/00 to try and make guardian's for class halls
bool spec_clan_torcalta(CHAR_DATA *ch)*/
bool spec_clan_vampires(CHAR_DATA *ch)
{
   DESCRIPTOR_DATA       *d;
   CHAR_DATA             *victim;
   char                  buf[MAX_STRING_LENGTH];
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
	   ...
	   || (IS_CLASS(victim, CLASS_VAMPIRE)))
	{
        ...
        if (ch->in_room!=victim->in_room)
        {
           act("$n senses an intruder and vanishes.",ch,NULL,NULL,TO_ROOM);
           act("You sense an intruder and vanish.",ch,NULL,NULL,TO_CHAR);

           char_from_room( ch );
           char_to_room(ch, victim->in_room);

           act("$n appears and attacks!",ch,NULL,NULL,TO_ROOM);
           act("You appear and attack!",ch,NULL,NULL,TO_CHAR);
        }
     
        act("$n slobbers and lunges at you.",ch,ch->long_descr,victim,TO_VICT);
        act("You roar and alert everybody.",ch,NULL,victim,TO_CHAR);
	  ...
	  ch->position == POS_STANDING)
        ...
        sprintf( buf,"%s is in the Vampire Elysium.\n\r", victim->name); 
        do_shout( ch , buf);
