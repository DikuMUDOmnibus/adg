...
void fwrite_char_old( CHAR_DATA *ch, FILE *fp )
{
	...
	fprintf( fp, "Power        %d %d\n",
        ...
        fprintf( fp, "Powers        %d %d\n",
            ch->pcdata->powerss[0],
            ch->pcdata->powerss[1] );

        fprintf( fp, "Powersm        %d %d\n",
            ch->pcdata->powersm[0],
            ch->pcdata->powersm[1] );

        fprintf( fp, "Powersd        %d %d\n",
            ch->pcdata->powersd[0],
            ch->pcdata->powersd[1] );

        fprintf( fp, "Powersde        %d %d\n",
            ch->pcdata->powersde[0],
            ch->pcdata->powersde[1] );

        fprintf( fp, "Powerst        %d %d\n",
            ch->pcdata->powerst[0],
            ch->pcdata->powerst[1] );
...
//fprintf( fp, "low4         %d\n",   ch->low4 			);
	...
	fprintf( fp, "Power        ");
        ...
        fprintf( fp, "Powers        ");
            for (sn=0; sn < 20; sn++)
            {
                fprintf(fp, "%d ", ch->pcdata->powerss[sn]);
            }
            fprintf(fp, "\n");
        ...
        fprintf( fp, "Powersm        ");
            for (sn=0; sn < 20; sn++)
            {
                fprintf(fp, "%d ", ch->pcdata->powersm[sn]);
            }
            fprintf(fp, "\n");
        ...
        fprintf( fp, "Powersd        ");
            for (sn=0; sn < 20; sn++)
            {
                fprintf(fp, "%d ", ch->pcdata->powersd[sn]);
            }
            fprintf(fp, "\n");
        ...
        fprintf( fp, "Powersde        ");
            for (sn=0; sn < 20; sn++)
            {
                fprintf(fp, "%d ", ch->pcdata->powersde[sn]);
            }
            fprintf(fp, "\n");
        ...
        fprintf( fp, "Powerst        ");
            for (sn=0; sn < 20; sn++)
            {
                fprintf(fp, "%d ", ch->pcdata->powerst[sn]);
            }
            fprintf(fp, "\n");

        fprintf( fp, "Principles        ");
            for (sn=0; sn < 20; sn++)
            {
                fprintf(fp, "%d ", ch->pcdata->principles[sn]);
            }
            fprintf(fp, "\n");
...
//    ch->pcdata->atm			= 0;
/*
    ch->pcdata->mdecap			= str_dup( "has been decapitated by" );
*/
...
*/
 ...
 strtime = ctime( &current_time );
    ...
    ch->pcdata->quest			= 0;
    ch->pcdata->kingdom			= 0;   
    ch->pcdata->dragonaff		= 0;
    ch->pcdata->dragonage		= 0;
    ch->pcdata->breath[0]		= 0;
    ch->pcdata->breath[1]		= 0;
    ch->pcdata->breath[2]               = 0;
    ch->pcdata->breath[3]               = 0;
    ch->pcdata->breath[4]               = 0;
    ch->pcdata->breath[5]               = 0;
    ch->pcdata->hatch			= 0;
    ch->pcdata->wolf			= 0;
    ch->pcdata->rank			= 0;
    ...
    for ( sn=0 ; sn < 20 ; sn++)
        ch->pcdata->powerss[sn]          = 0;

    for ( sn=0 ; sn < 20 ; sn++)
        ch->pcdata->powersm[sn]          = 0;

    for ( sn=0 ; sn < 20 ; sn++)
        ch->pcdata->powersd[sn]          = 0;

    for ( sn=0 ; sn < 20 ; sn++)
        ch->pcdata->powersde[sn]          = 0;

    for ( sn=0 ; sn < 20 ; sn++)
        ch->pcdata->powerst[sn]          = 0;

    for ( sn=0 ; sn < 20 ; sn++)
        ch->pcdata->principles[sn]      = 0;
...
*/
    ...
    ch->pcdata->powerss[0]              = 0;
    ch->pcdata->powerss[1]              = 0;
    ch->pcdata->powersm[0]               = 0;
    ch->pcdata->powersm[1]               = 0;
    ch->pcdata->powersd[0]               = 0;
    ch->pcdata->powersd[1]               = 0;
    ch->pcdata->powersde[0]               = 0;
    ch->pcdata->powersde[1]               = 0;
    ch->pcdata->powerst[0]               = 0;
    ch->pcdata->powerst[1]               = 0;
    ch->pcdata->willpower[0]		= 50;
    ch->pcdata->willpower[1]		= 50;
    ch->pcdata->resist[0]		= 0;
    ch->pcdata->resist[1]		= 0;
...
void fread_char_old( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'D':
	    ...
	    KEY( "Description",	ch->description,	fread_string( fp ) );
            KEY( "Dragonaff",      ch->pcdata->dragonaff, fread_number( fp ) )
	...
	case 'I':
	    ...
	    KEY( "Itemaffect",    ch->itemaffect,	fread_number( fp ) );
            KEY( "Itemaffects",	  ch->itemaffects,	fread_number( fp ) );
...
*/
	...
	case 'P':
		...
		break;
            ...
            {
                ch->pcdata->powerss[0] = fread_number( fp );
                ch->pcdata->powerss[1] = fread_number( fp );
                fMatch = TRUE;
                break;
            }

            if ( !str_cmp( word, "Powersm" ) )
            {
                ch->pcdata->powersm[0] = fread_number( fp );
                ch->pcdata->powersm[1] = fread_number( fp );
                fMatch = TRUE;
                break;
            }

            if ( !str_cmp( word, "Powersd" ) )
            {
                ch->pcdata->powersd[0] = fread_number( fp );
                ch->pcdata->powersd[1] = fread_number( fp );
                fMatch = TRUE;
                break;
            }

            if ( !str_cmp( word, "Powersde" ) )    
            {
                ch->pcdata->powersde[0] = fread_number( fp );
                ch->pcdata->powersde[1] = fread_number( fp );
                fMatch = TRUE;
                break;
            }

            if ( !str_cmp( word, "Powerst" ) )
            {
                ch->pcdata->powerst[0] = fread_number( fp );
                ch->pcdata->powerst[1] = fread_number( fp );
                fMatch = TRUE;
                break;
            }
...
*/
	...
	case 'T':
		...
		break;
            ...
            if ( !str_cmp( word, "Titlec" ) )
            {
                ch->pcdata->titlec = fread_string( fp );
                if ( isalpha(ch->pcdata->titlec[0])
                ||   isdigit(ch->pcdata->titlec[0]) )
                {
                    sprintf( buf, " %s", ch->pcdata->titlec );
    ...
    *obj		= obj_zero;
	...
	case 'I':
	    KEY( "ItemType",	obj->item_type,		fread_number( fp ) );
            KEY( "Immune",      obj->immune,            fread_number( fp ) );
	    break;
...
//	KEY( "Atm",		ch->pcdata->atm, fread_number( fp) );
	...
	case 'C':
	    ...
	    KEY( "Class",  	ch->class,       	fread_number( fp ) );
            KEY( "Classt",      ch->class_2,            fread_number( fp ) );
            KEY( "Classtt",	ch->class_3,		fread_number( fp ) );
...
*/
	...
	case 'P':
		...
		break;
            ...
            {
                for ( sn = 0; sn < 20 ; sn++ )
                        ch->pcdata->powerss[sn] = fread_number( fp );
            ...
            {
                for ( sn = 0; sn < 20 ; sn++ )
                        ch->pcdata->powersm[sn] = fread_number( fp );
            ...
            {
                for ( sn = 0; sn < 20 ; sn++ )
                        ch->pcdata->powersd[sn] = fread_number( fp );
            ...
            {
                for ( sn = 0; sn < 20 ; sn++ )
                        ch->pcdata->powersde[sn] = fread_number( fp );
            ...
            {
                for ( sn = 0; sn < 20 ; sn++ )
                        ch->pcdata->powerst[sn] = fread_number( fp );
                
                fMatch = TRUE;
                break;
            }

            if ( !str_cmp( word, "Principles" ) )
            {
                for ( sn = 0; sn < 20 ; sn++ )
                        ch->pcdata->principles[sn] = fread_number( fp );
...
*/
	...
	if (ch->trust >= 4 && ch->trust <=6)
	{
		...
		ch->pcdata->powersm[MPOWER_RUNE0] = ch->pcdata->runes[0];
		ch->pcdata->powersm[MPOWER_RUNE1] = ch->pcdata->runes[1];
		ch->pcdata->powersm[MPOWER_RUNE2] = ch->pcdata->runes[2];
		ch->pcdata->powersm[MPOWER_RUNE3] = ch->pcdata->runes[3];
