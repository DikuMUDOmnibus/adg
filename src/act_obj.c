...
void do_itempurge( CHAR_DATA *ch, char *argument )
{
//   char buf[MAX_STRING_LENGTH];
   int oldlevel, oldtrust;


    if (IS_SET(ch->newbits, NEW_PURGE) && !IS_IMMORTAL(ch)) return;

    oldlevel = ch->level;
    oldtrust = ch->trust;

    if(ch->level == 6 && ch->trust == 6)
    {
    oldlevel = 5;
    oldtrust = 5;
    }
...
*/
    ch->level = oldlevel;
    ch->trust = oldtrust;
    send_to_char("#rYou have purged all your items for the Change over.\n\r#n",ch);
    send_to_char("#rPlease read the Announce Board first then all the rest.#n",ch);
...
//   char buf[MAX_STRING_LENGTH];
/* Changed by Toaster 07/02/00 so Immortals can make any number of newbiepacks.
 * Changed by Kyrlin 03/03/00 to include new newbieeq.
 */
        if (IS_SET(ch->newbits, NEWBIE_PACK) && !IS_IMMORTAL(ch))
        {
...
void npack( CHAR_DATA *ch, char *argument )
{
        ...
        else if (!str_cmp( arg,"dagger"))       vnum = 31013;
        else if (!str_cmp( arg,"belt" ))        vnum = 31014;
        else if (!str_cmp( arg,"cloak"))        vnum = 31015;
        else if (!str_cmp( arg,"mask" ))        vnum = 31016; 
...
/* Changed Toaster 21/03/00 to stop voodoo dolls being hidden*/
...
void do_eat( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC(ch) ) ch->pcdata->quest += obj->value[0];
        sprintf(buf,"%s ate a token worth %d qp.",ch->name,obj->value[0]);
        log_string( buf );
	break;
...
/*
	...
	&& !IS_POLYAFF(ch, POLY_ZULO) && !IS_POLYAFF(ch, POLY_NEWZULO) && !IS_POLYAFF(ch, POLY_SPIDERFORM))
    {
...
//	     obj->points > 0 && str_cmp(ch->name, obj->questmaker) ) continue;
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
	...
	{
	 sprintf( buf, "You receive a refund of 5000 demonic points." );
         act(buf,ch,obj,NULL,TO_CHAR);
	 ch->pcdata->stats[DEMON_CURRENT] += 5000;
	}
	else if(obj->specpower == ARM_RED)
	{
	 sprintf( buf, "You receive a refund of 10000 demonic points." );
         act(buf,ch,obj,NULL,TO_CHAR);
         ch->pcdata->stats[DEMON_CURRENT] += 10000;
        }
	 else if(obj->specpower == ARM_COPPER)
        { 
	 sprintf( buf, "You receive a refund of 15000 demonic points." );
         act(buf,ch,obj,NULL,TO_CHAR);
         ch->pcdata->stats[DEMON_CURRENT] += 15000;
        }
	 else if(obj->specpower == ARM_BRONZE)
        { 
	 sprintf( buf, "You receive a refund of 20000 demonic points." );
         act(buf,ch,obj,NULL,TO_CHAR);
         ch->pcdata->stats[DEMON_CURRENT] += 20000;
        }
	 else if(obj->specpower == ARM_BRASS)
        { 
         sprintf( buf, "You receive a refund of 25000 demonic points." );
         act(buf,ch,obj,NULL,TO_CHAR);
         ch->pcdata->stats[DEMON_CURRENT] += 25000;
        }
}
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if(is_safe(ch,victim))
    {
        ...
        sprintf( buf, "%s is trying to steal in a saferoom!", ch->name );
        do_info( ch, buf );
        return;
    }
...
void do_list( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = keeper->carrying; obj; obj = obj->next_content )
	{
		...
		{
		    found = TRUE;
		    send_to_char( "{D[{m>{W  Price  {m<{D]{w Item\n\r", ch );
		}

		sprintf( buf, "{D[{m>{W %7d {m<{D]{w %s.{x\n\r",
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( obj = get_obj_carry( ch, arg ) ) == NULL )
    {
	act( "#w$n tells you '#yYou don't have that item#w'.#n",keeper, NULL, ch, TO_VICT );
	ch->reply = keeper;
	return;
    }
...
void do_activate( CHAR_DATA *ch, char *argument )
{
    ...
    if(ch->fight_timer > 0)
    {
	send_to_char( "Not with a fight timer.",ch);
	return;
    }
	...
	do_look(ch,"auto");
       ...
       if (!IS_SET(obj->quest, QUEST_ARTIFACT) &&
	    ...
	    sprintf( buf, "%s has used a transporter to a HQ, Decap Them!", ch->name );  
	    sprintf( log_buf, "%s has used a transporter to a HQ!", ch->name );  
            do_info( ch, buf );
	    log_string( log_buf );
	    do_paradox(ch, "self");
            char_from_room(ch);
/*Changed by Toaster 04/02/00 to make folk appear in healers room 
...
void do_recharge( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(arg2,"reset") && ch->pcdata->quest > 100)
    { quest_object( ch, obj ); ch->pcdata->quest -= 100; stc("Quest card has been reset.\n\r",ch); return; }
    ...
    act("$P makes a few clicks and returns $p.",ch,obj,qobj,TO_ROOM);
    value = obj->level;
    if (value < 1) value = 1; else if (value > 500) value = 500;
    value += ch->race;
    obj = create_object(get_obj_index( OBJ_VNUM_PROTOPLASM ), 0);
    obj->value[0] = value;
    obj->level = value;
...
/*
 ch->pcdata->quest += value; 
       sprintf(buf,"You gain an additional %d qp from double questpoints!\n\r",value);
       stc(buf,ch);
       ch->pcdata->score[SCORE_QUEST] += value;*/
    value = obj->level;
    if (value < 1) value = 1; else if (value > 500) value = 500;
...
void quest_object( CHAR_DATA *ch, OBJ_DATA *obj )
{
    static const sh_int quest_selection[] = 
    {
         102,  
         9201, 9225,  605, 1329, 2276, 5112, 6513, 6517, 6519, 5001,
         5005, 5011, 5012, 5013, 2902, 1352, 2348, 2361,31008, 5011,
         5012, 5013, 2902, 1352, 2348, 2361,31008,  300,  303,  307,
         7216, 1100,  100,30315, 5110, 6001, 3050,  301, 5230,30302,
          663, 7303, 2915, 2275, 8600, 8601, 8602, 8603, 5030, 9321,
         6010, 1304, 1307, 1332, 1333, 1342, 1356, 1361, 2304, 2322,
         2331, 2382, 27706, 8005, 5300, 5302, 5309, 5310, 5311, 4000,
          601,  664,  900,  906,  923,  311, 7203, 7206, 1101, 5214,
         5223, 5228, 2804, 1612, 5207, 9302, 5301, 5224, 7801, 9313,
         6304, 2003, 3425, 3423, 608,  1109,30319, 8903, 9317, 9307,
         4050,  911, 2204, 4100, 3428,  310, 5113, 3402, 5319, 6512,
        ...
        30325, 6106, 2003, 7190, 9322, 1384, 3412, 2342, 1374, 2210,
         2332, 2901, 7200, 7824, 3410, 2013, 1510, 8306, 3414, 2005,
        11007,11008,11012,11015,11101,11104,11107,11111,11114,11117,
        11120,11121,11122,11123,11124,11125, 3000, 3001, 3002, 3003,
         3004, 3032, 3033, 1345, 2905, 2911, 3002, 3005, 3425, 3611,
         4706, 5003, 5228, 6000, 6509, 6600, 6646, 7223, 7310, 7704,
         7714, 8119, 9105, 9211, 9213, 9218, 9228, 9231, 9302, 9306,
        31106,31108,31109,31116,31118,31119,31126,31128,31129,31136,
        31138,31139,31140,11106,11108,11109,11116,11118,11119,11126,
        11128,11129,11136,11138,11139,11140,11144, 7700, 7703, 7712,
         7718, 8000, 5237, 5239, 5251, 5310, 5312, 5235, 6007, 6308,
         6502, 6511, 6512, 2271, 2299, 2300, 2301, 2302, 2306, 2307,
         2310, 2311, 2317, 2318, 2319, 2324, 2340, 2345, 2357, 2358,
	 1311,  666, 1307, 2456, 1313,  304, 6811, 19208, 7718, 9108
    };
    int object,obj2,obj3,obj4;
    OBJ_INDEX_DATA *pObjIndex;
	...
	|| obj->value[2] == obj->value[3] )
    {
    object = number_range(1, 271);
    obj2 = number_range(1, 271);
    obj3 = number_range(1, 271);
    obj4 = number_range(1, 271);

    if ( ( pObjIndex = get_obj_index( object ) ) ||
         ( pObjIndex = get_obj_index( obj2   ) ) ||
         ( pObjIndex = get_obj_index( obj3   ) ) || 
         ( pObjIndex = get_obj_index( obj4   ) ) == NULL )
    {
            bug("QUEST: %d don't exist.",object);
            object = number_range(0,20);
    }

    if (object < 1 || object > 271) object = 0;
    obj->value[0] = quest_selection[object];

    if (obj2 < 1 || obj2 > 271) obj2 = 0;
    obj->value[1] = quest_selection[obj2];

    if (obj3 < 1 || obj3 > 271) obj3 = 0;
    obj->value[2] = quest_selection[obj3];

    if (obj4 < 1 || obj4 > 271) obj4 = 0;
    obj->value[3] = quest_selection[obj4];
    }
...
void do_complete( CHAR_DATA *ch, char *argument )
{
    ...
    if (rnum == 10) {
    stc("You attempt to touch the item onto the card, but the item suddenly vanishes!\n\r",ch);
    obj_from_char(obj);
    extract_obj(obj);
    return;
    }
