...
void violence_update( void )
{
  ...
  victim = ch->blinkykill;
	...
	{
	    act("$n pops back into existence next to $N.", ch, NULL,ch->blinkykill, TO_NOTVICT);
	    ...
	    act("#wYou exclaim '#rMuhahahahahaha!#w'.#n",ch,NULL,NULL,TO_CHAR);
	    act("#w$n exclaims '#rMuhahahahahaha!#w'.#n",ch,NULL,NULL,TO_ROOM);
	    ch->blinkykill = NULL;
...
/*
	 if (!IS_NPC(ch) && !IS_IMMORTAL(ch) && ch->race < 20 && IS_SET(ch->in_room->room_flags, ROOM_SAFE) 
	    && (ch->in_room->vnum != 2) && ch->race > 0 && ch->safe_timer >= 1)
            {
                    ch->safe_timer++;
                    if (ch->safe_timer == number_range(60,65))
                    {
                        send_to_char("You have been safe long enough. GET OUT\n\r",ch);
                        char_from_room(ch); 
                        char_to_room(ch, get_room_index(3001));
                        do_look(ch,"");
	 	        act("$n appears to have been kicked out of a safe room.",ch,NULL,NULL,TO_ROOM);
                    }
            }*/
...
/*
	...
	&& IS_SET(ch->pcdata->powersd[1], DPOWER_ARMS))
...
if ( ch->fighting != victim || dt == gsn_backstab || dt == gsn_shiroken
    || dt == gsn_garotte || dt == gsn_headbutt) return;
	...
	if (IS_CLASS(ch, CLASS_DRAGON)	&& IS_SET(ch->pcdata->dragonaff, DRA_TAIL)) 
                one_hit( ch, victim, gsn_sweep, 0);
	if( IS_CLASS(ch, CLASS_DRAGON) && IS_SET(ch->pcdata->dragonaff, DRA_WINGS))
                one_hit( ch, victim, gsn_buffet, 0);
        if( IS_CLASS(ch, CLASS_DRAGON) && IS_SET(ch->pcdata->dragonaff, DRA_FANGS))   
	...
	if (IS_CLASS(ch, CLASS_DRAGON) && !str_cmp(ch->pcdata->breath[x], "frost")
	 && ( (number_range(1,3) == 1))) {
                 ...
                 sdam = number_range(750,1500);
                 } else { 
                 sdam = number_range(250,500); }
                 if (victim->hit < sdam) return;
                 if ( IS_AFFECTED(victim, AFF_SANCTUARY)) sdam = sdam / 2;
                 sprintf(buf,"{g%s's frost breath {C*{c={D-{C ANNHILATES {D-{c={C*{g you! {D[{w%d{D]{x\n\r",ch->name,sdam); 
                 if (!IS_NPC(victim)) stc(buf,victim);
                 sprintf(buf,"{gYour frost breath {C*{c={D-{C ANNHILATES {D-{c={C*{g them! {D[{w%d{D]{x\n\r",sdam);
                 stc(buf,ch);
                 victim->hit -= sdam;}
	...
	if (IS_CLASS(ch, CLASS_DRAGON) && !str_cmp(ch->pcdata->breath[x], "fire")
	 && ( (number_range(1,3) == 1))){   
                 ...
                 if (victim->hit < sdam) return;
                 if ( IS_AFFECTED(victim, AFF_SANCTUARY)) sdam = sdam / 2;
                 sprintf(buf,"{g%s's fire breath {R*{r={D-{R MUTILATES {D-{r={R*{g you! {D[{w%d{D]{x\n\r",ch->name,sdam);
                 if (!IS_NPC(victim)) stc(buf,victim);
                 sprintf(buf,"{gYour fire breath {R*{r={D-{R MUTILATES {D-{r={R*{g them! {D[{w%d{D]{x\n\r",sdam);
                 stc(buf,ch);
                 victim->hit -= sdam;}
	...
	if (IS_CLASS(ch, CLASS_DRAGON) && !str_cmp(ch->pcdata->breath[x], "lightning")
	 && ( (number_range(1,3) == 1))) {   
                 ...
                 if (victim->hit < sdam) return;
                 if ( IS_AFFECTED(victim, AFF_SANCTUARY)) sdam = sdam / 2;
                 sprintf(buf,"{g%s's lightning breath {Y*{y={D-{Y SHOCKS {D-{y={Y*{g you! {D[{w%d{D]{x\n\r",ch->name,sdam);
                 if (!IS_NPC(victim)) stc(buf,victim);
                 sprintf(buf,"{gYour lightning breath {Y*{y={D-{Y SHOCKS {D-{y={Y*{g them! {D[{w%d{D]{x\n\r",sdam);
                 stc(buf,ch);
                 victim->hit -= sdam;}


	if (IS_CLASS(ch, CLASS_DRAGON) && !str_cmp(ch->pcdata->breath[x], "acid")
	 && ( (number_range(1,3) == 1))){   
                 if (IS_SET(ch->pcdata->dragonaff, DRA_LUCK)) {
                 sdam = number_range(750,1500);
                 } else {
                 ...
                 if (victim->hit < sdam) return;
                 if ( IS_AFFECTED(victim, AFF_SANCTUARY)) sdam = sdam / 2;
                 sprintf(buf,"{g%s's acid breath {G*{g={D-{G DISGUSTS {D-{g={G*{g you! {D[{w%d{D]{x\n\r",ch->name,sdam);
                 if (!IS_NPC(victim)) stc(buf,victim);
                 sprintf(buf,"{gYour acid breath {G*{g={D-{G DISGUSTS {D-{g={G*{g them! {D[{w%d{D]{x\n\r",sdam);
                 stc(buf,ch);
                 victim->hit -= sdam;}

	if (IS_CLASS(ch, CLASS_DRAGON) && !str_cmp(ch->pcdata->breath[x], "gas")
	 && ( (number_range(1,3) == 1)))
		spell_gas_breath(skill_lookup( "gas breath" ),ch->pcdata->dragonage * 20, ch, victim );
    }          

/* Changed Toaster 20/04/00 so only riposte when wielding anything, yeh
yeh so i'll make it only with slice weapons or something too later */
    if (IS_HIGHLANDER(ch) && get_eq_char(ch, WEAR_WIELD) != NULL)
    {
        one_hit( ch, victim, gsn_riposte, 0);dt += TYPE_ADD_AGG;
    }
  ...
  if(IS_WEREWOLF(ch))
  {
	...
	get_eq_char(ch, WEAR_WIELD ) == NULL && get_eq_char(ch, WEAR_HOLD) == NULL)
	{
  ...
  if(IS_DEMON(ch))
  {
      ...
      if(IS_SET(ch->warp, WARP_WINGS)  && number_range(1,3) == 1)
        one_hit( ch, victim, gsn_buffet, 0 );  
	...
	dt += TYPE_ADD_AGG;
      ...
      if(IS_DEMAFF(ch, DEM_TAIL)  && number_range(1,3) == 1)
  ...
  if (IS_CLASS(ch, CLASS_NINJA))
  {
	...
	one_hit(ch, victim, gsn_claws, 0);
      ...
      if ( ch->pcdata->powerss[BLACK_BELT] > 5)
        multi_hit(ch, victim, gsn_shiroken);
  }

/*
 if ( IS_DROW(ch) && IS_SET(ch->pcdata->powersd[1], DPOWER_ARMS) )
 {
     int ii;
...
if ( IS_CLASS(ch, CLASS_DROW) && IS_SET(ch->pcdata->powersd[1], DPOWER_ARMS))
{
   if (number_range(1,2) == 1) {
       one_hit( ch, victim, gsn_spiderform, 6 );
       one_hit( ch, victim, gsn_spiderform, 6 );
       one_hit( ch, victim, gsn_spiderform, 6 );
       one_hit( ch, victim, gsn_spiderform, 6 );
   } else {
   ...
   one_hit( ch, victim, gsn_tail, 0 );
	...
	spell_poison( gsn_poison, (ch->level*number_range(5,10)),ch,victim );
    else if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powerss[NPOWER_NINGENNO] > 4)
	spell_poison( gsn_poison, (ch->level*number_range(5,10)),ch,victim );
...
int number_attacks( CHAR_DATA *ch )
{
	...
	if (ch->level >= 200)  count++;
	if (ch->level >= 300)  count++;
	if (ch->level >= 400)  count++;
	if (ch->level >= 500)  count += 2;
	if (ch->level >= 700)  count += 2;
	if (ch->level >= 900)  count += 2;
	if (ch->level >= 1000) count = ( ch->level / 50 );
	return count;
    }

    dten = number_percent() + number_percent() + number_percent();

    if ( IS_STANCE(ch, STANCE_VIPER) && dten < 
	ch->stance[STANCE_VIPER]) count += 1;
    else if ( IS_STANCE(ch, STANCE_MANTIS) && dten < 
	ch->stance[STANCE_MANTIS]) count += 1;
    else if ( IS_STANCE(ch, STANCE_RABBIT) && dten < 
	ch->stance[STANCE_RABBIT]) count += 2;
    else if ( IS_STANCE(ch, STANCE_TIGER) && dten < 
	ch->stance[STANCE_TIGER]) count += 2;
	  else if ( IS_STANCE(ch, STANCE_BEAR) && dten < 
	ch->stance[STANCE_BEAR]) count += 3;
	...
	if (is_affected(ch, skill_lookup("haste")))
		...
		count += (ch->pcdata->powersde[VAM_CELE]*2)/3;
	else if ( IS_CLASS(ch, CLASS_MONK) && ch->monkab[COMBAT] > 0 )
//		count += ch->monkab[COMBAT];
		count += (ch->monkab[COMBAT]*2)/3;
        else if (IS_CLASS(ch, CLASS_DRAGON) && IS_DRAPOWER(ch, DRA_CONFUSE) )
                count += 1;
   	else if (IS_DEMON(ch) && IS_DEMPOWER(ch, DEM_SPEED) )
		count += 1;
	else if ( IS_SET(ch->warp, WARP_QUICKNESS) ) 
		count += 1;
	else if (IS_DROW(ch) && IS_SET(ch->special, SPC_DROW_WAR))
		count += 3;
	else if (!IS_NPC(ch) && IS_NINJA(ch) && ch->pcdata->powerss[NPOWER_CHIKYU] > 2)
                count += 2;
        else if (!IS_NPC(ch) && IS_NINJA(ch) && ch->pcdata->powerss[BLACK_BELT] > 3)
                count += 1;
	  ...
	  count++;
        ...
        if (IS_CLASS(ch, CLASS_DROW) && IS_SET(ch->pcdata->powersd[1],DPOWER_SPEED))
	      count += 1;
    }
...
//	thac0_00 = 20;
    ...
    if ( IS_NPC(ch) )
    {
//	dam = number_range( ch->level, ch->level * 4 / 2 );
        dam = 5.5;
	if ( wield != NULL )
	    dam += dam * 1;
    }
...
//    bool doubled = FALSE;
        ...
        if ( IS_CLASS(ch, CLASS_VAMPIRE) )
	{
	    if ( ch->pcdata->powersde[VAM_POTE] > 0 )
                mult += ( ch->pcdata->powersde[VAM_POTE] / 5 );
	...
	if ( IS_HIGHLANDER(ch) )
	{
	if( (wield != NULL) && (wield->pIndexData->vnum == 29695) )
	    mult += 1;
	}
	...
	{ 
	    ...
	    if (IS_GAR1(ch, WOLF_RAZORCLAWS ) ) mult += 1;

/*Toast changed 27/08/01 since getoffenris power gave +3 mult and fianna gave 
+2 wtf is all that about*/

            if (ch->gifts[FIANNA] > 2) mult += .2;
	...
	if ( IS_CLASS(ch, CLASS_DROW) )
        ...
        {
             mult += ch->pcdata->dragonage / 2;
             if ( IS_DRAPOWER(ch, DRA_CONFUSE)) mult += 1;
        }
	if ( IS_CLASS(ch, CLASS_DEMON) )
	{
	    if ( ch->in_room->vnum == 30000 ) mult += 3;
	...
	if ( IS_CLASS(ch, CLASS_NINJA) )
	{
	    if ( ch->pcdata->powerss[NPOWER_CHIKYU] > 1 )
	        mult += 3;
            if ( ch->pcdata->powerss[BLACK_BELT] > 2)
                mult += ch->pcdata->powers[BLACK_BELT] / 2;
	}
    }
   ...
   if ( !IS_NPC( victim ) )
	...
	if ( IS_CLASS(victim, CLASS_MAGE) )
	{
        ...
        {
            if ( IS_DRAPOWER(victim, DRA_TUFF) )
                reduct += 1.4;
        }
	if ( IS_CLASS(victim, CLASS_DEMON) )
	{
	    if ( IS_DEMPOWER(victim, DEM_TOUGH) )
	        reduct += 1.4;
	    if ( IS_SET(victim->warp, WARP_WEAK))
		reduct -=  .5;
            if ( IS_SET(victim->warp, WARP_SBODY))
                reduct +=  1;
	    if ( IS_SET(victim->warp, WARP_GBODY))
                reduct -=  1;
...
/* Toast Changed because reduct +ch is silly thats like dam /8 right off*/
	       reduct += victim->chi[CURRENT]/10;
	...
	if ( IS_CLASS(victim, CLASS_NINJA) )
	{
	    reduct += 1.4;
/*
            if (ch->pcdata->powerss[BLACK_BELT] == 9)
            reduct += 1;*/
	}
...
//	    if ( IS_SET(ch->newbits, NEW_DROWHATE) )
            if (IS_SET(ch->pcdata->powersd[1], DPOWER_DROWHATE))
	        max_dam += 750;
            if (IS_SET(ch->pcdata->powersd[1], DPOWER_ARMS))
                max_dam += 350;
	}
	...
	if ( IS_CLASS(ch, CLASS_DEMON) )
	{
        ...
        if ( IS_CLASS(ch, CLASS_DRAGON) )
         {
            max_dam += 1000;
            if (IS_DRAPOWER(ch, DRA_CONFUSE)) max_dam += 500;
               if (IS_DRAPOWER(ch, DRA_TUFF)) max_dam += 500;
            max_dam += ch->pcdata->dragonage * 100;
        }
	...
	if ( IS_CLASS(ch, CLASS_VAMPIRE) )
	{
	    ...
	    else if ( ch->pcdata->stats[UNI_GEN] < 3 ) max_dam = 1500;	    
	    ...
	    if ( ch->pcdata->powersde[VAM_POTE] > 0 )
	      max_dam += ( 100 * ch->pcdata->powersde[VAM_POTE] );
	    if ( ch->pcdata->rank == AGE_METHUSELAH ) max_dam += 500;
	    else if ( ch->pcdata->rank == AGE_ELDER ) max_dam += 300;
	    else if ( ch->pcdata->rank == AGE_ANCILLA ) max_dam += 200;
	    else if ( ch->pcdata->rank == AGE_NEONATE ) max_dam += 100;
	    else if ( ch->pcdata->rank == AGE_CHILDE ) max_dam += 50;
            else max_dam += 0;
	...
	if ( IS_CLASS(ch, CLASS_NINJA) )
	{
	    ...
	    if ( ch->pcdata->powerss[NPOWER_CHIKYU] >= 6 && ch->pcdata->powerss[HARA_KIRI] > 0 )
	        max_dam += 600;
	    if ( ch->pcdata->powerss[NPOWER_CHIKYU] >= 2 )
	        max_dam += 500;
            max_dam += ch->pcdata->powerss[BLACK_BELT] * 100;
	}
	if ( IS_CLASS(ch, CLASS_MAGE) )
	{
	    max_dam = 1500;
	...
	if ( IS_CLASS(ch, CLASS_WEREWOLF) )
	{
	    ...
	    if ( ch->gifts[GETOFFENRIS] >= 3 )
	        max_dam += 300;
	    if (IS_ITEMAFF(ch, ITEMA_SUNSHIELD) && IS_VAMPIRE(victim)) max_dam += 500;
	}
...
/* New Toast 05/06/01 */
    ...
    {  
	    ...
	    else if (ch->stance[0] == STANCE_COBRA)  max_dam += 450;
	    else if (ch->stance[0] == STANCE_EAGLE)  max_dam += 200;
	    else if (ch->stance[0] == STANCE_COUGAR)  max_dam += 250;
	    else if (ch->stance[0] == STANCE_BEAR)  max_dam += 250;

    }

    if(!IS_NPC(victim) && char_ac(victim) > 0)
    {
	max_dam += char_ac(victim);
    }
    ...
    if (IS_NPC(ch) || ch->stance[0] != STANCE_MONKEY)
    {
      ...
      else if (victim->stance[0] == STANCE_RABBIT) max_dam -= 250;
      else if (victim->stance[0] == STANCE_BEAR) max_dam -= 500;
      else if (victim->stance[0] == STANCE_COUGAR) max_dam -= 250;
      else if (victim->stance[0] == STANCE_COBRA) max_dam -= 250;
    }
...
/* removed because it was in the wrong place, moved to the correct area.
...
*/
	...
	if( IS_SHAKRIN(victim) && dam > 1 ) dam = 1;
	...
	if ( IS_AFFECTED(victim, AFF_PROTECTG) && IS_GOOD(ch) && dam > 1 )
	    dam -= dam * 0.25;
	...
	if ( dt >= TYPE_HIT )
	{
	 ...
	 if ( IS_NPC(ch) && number_percent( ) < ch->level * 0.5 )
		...
		return;
	    if ( !IS_NPC(victim) && IS_STANCE(victim, STANCE_MANTIS) && 
		victim->stance[STANCE_MANTIS] > 100 && !can_counter(ch)
		&& !can_bypass(ch,victim) && check_parry( ch, victim, dt ))
		return;
	    if ( !IS_NPC(victim) && IS_STANCE(victim, STANCE_RABBIT) && 
    ...
    if ( IS_MONK(ch) && ch->monkab[SPIRIT] >= 2 )
    {
        if (IS_SET(ch->newbits, NEW_NATURAL) )
        {
            stc("#yYou absorb the natural imbalances of your opponent.#n\n\r",ch);
            act("#y$n absorbs the natural imbalances of you attack.#n",ch,NULL,victim,TO_VICT);
            if ( dam > 2000 ) dam /= 2;
            ch->hit += dam;
            REMOVE_BIT(ch->newbits, NEW_NATURAL);
            return;
        }
    }
...
// victim->hit -= dam;
	...
	if (IS_CLASS(ch, CLASS_DROW) && IS_NPC(victim) && !IS_SET(victim->act, ACT_NOEXP ))
		...
		{
		    ch->pcdata->stats[DEMON_CURRENT] += victim->level*1.5;
		    ch->pcdata->stats[DEMON_TOTAL] += victim->level*1.5;
		}
	    }
	    }
	...
	if ( !IS_NPC(ch) )
	{
        ...
        {
            if ( IS_SET(ch->newbits, PLR_AUTOCON) )
                do_consume( ch, "" );
        }
	return;
    }
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ((chao_info.chs == 1)) return FALSE;

    if (!IS_NPC(victim) && IS_SET(victim->flag2, AFF2_BLINK_1ST_RD))
	return TRUE;
    ...
    if ( IS_SET(ch->in_room->room_flags,ROOM_SAFE) && victim->race < 20)
    {
...
//    chance -= (char_hitroll(ch) * 0.3);
        ...
        if ( IS_CLASS(ch, CLASS_VAMPIRE) )
	{
	    if ( ch->pcdata->powersde[VAM_CELE] > 0 )
	        chance -= ( 4 * ch->pcdata->powersde[VAM_CELE] );
	}
	if ( IS_CLASS(ch, CLASS_NINJA) )
	{
	    if ( ch->pcdata->powerss[NPOWER_CHIKYU] >= 6 && ch->pcdata->powerss[HARA_KIRI] > 0 )
	    ...
	    if ( ch->pcdata->powerss[NPOWER_CHIKYU] > 2 )
	        chance -= 20;
	    if ( ch->pcdata->powerss[NINJAKI] > 4 )
	        chance -= ( ch->pcdata->powerss[NINJAKI] / 4 );
	} 

        if ( IS_CLASS(ch, CLASS_DRAGON) )
        {
            if ( IS_DRAPOWER(ch, DRA_CONFUSE) )
                chance -= 15;
        }
	if ( IS_CLASS(ch, CLASS_DEMON) )
	{
        ...
        if ( IS_CLASS(victim, CLASS_VAMPIRE) )
	{
	    if ( victim->pcdata->powers[VAM_CELE] > 0 )
	        chance += ( 4 * victim->pcdata->powers[VAM_CELE] );
	}
	if ( IS_CLASS(victim, CLASS_NINJA) )
	{
	    if ( victim->pcdata->powerss[NPOWER_CHIKYU] >= 6 && victim->pcdata->powerss[HARA_KIRI] > 0 )
	        chance += 25;
	    if ( victim->pcdata->powerss[NPOWER_CHIKYU] > 2 )
	        chance += 20;
	    if ( victim->pcdata->powerss[NINJAKI] > 4 )
	        chance += ( victim->pcdata->powerss[NINJAKI] / 4 );
	}
        if ( IS_CLASS(victim, CLASS_DRAGON) )
        {
            if ( IS_DRAPOWER(victim, DRA_CONFUSE) )
                chance += 15;
        }
	if ( IS_CLASS(victim, CLASS_DEMON) )
	{
	...
	if ( IS_CLASS(victim, CLASS_DROW) )
	{
	    if ( IS_SET(victim->pcdata->powersd[1], DPOWER_ARMS) )
	        chance += 20;
		chance += 40;
	}
	if ( IS_CLASS(victim, CLASS_MAGE) )
   ...
   if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_MONK) )
    {
     if ( victim->monkblock / 4 >= 15 )
        chance += victim->monkblock / 5;
    }
    ...
    if ( IS_AFFECTED(ch, AFF_WEBBED) ) chance += 15;
    if ( IS_AFFECTED(victim, AFF_WEBBED) ) chance -= 15;
  ...
  /* Some shit for Earth Embrace )Mage Power(, works on NPC's.. */
    ...
    if (!IS_NPC(ch) && ch->pcdata->powersde[VAM_CELE] > 0 && 
    ...
    if (!IS_NPC(victim) && IS_SET(ch->flag2, AFF2_BLINK_1ST_RD))
	chance -= 30;
    ...
    if ( IS_NPC(victim) )
    {
      if ( victim->level < 100 )
        chance = 15;
      if ( victim->level >= 100 )
        chance = ( (victim->level/10) + number_range(10,20) );
      if ( victim->level >= 500 )
        chance = 50;
    ...
    if( !IS_NPC(victim) && victim->move < 5 )
        return FALSE;
    if( !IS_NPC(victim) )
    {
    move = 0;
    if (IS_STANCE(victim, STANCE_MANTIS)) move += 10;
    if (IS_STANCE(victim, STANCE_DRAGON)) move += 10;
    if (IS_STANCE(victim, STANCE_TIGER)) move += 10;
    if (IS_STANCE(victim, STANCE_MONKEY)) move += 10;
    if (IS_STANCE(victim, STANCE_SWALLOW)) move += 10;
    if (IS_STANCE(victim, STANCE_BEAR)) move += 15;
    if (IS_STANCE(victim, STANCE_EAGLE)) move += 15;
    if (IS_STANCE(victim, STANCE_COBRA)) move += 15; 
    if (IS_STANCE(victim, STANCE_RABBIT))move += 15;
    if (IS_STANCE(victim, STANCE_COUGAR)) move += 15;
    victim->move -= move;
    }
    ...
    {
	sprintf(buf,"#rParry: %d\n\r", chance);
	send_to_char(buf,ch);
    }

    if (IS_CLASS(ch, CLASS_MONK) && IS_SET(ch->newbits, NEW_MONKADAM)) hands = TRUE;
	...
	if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_MONK) && victim->monkblock <
	    150 && number_range(1,2)==1)
      victim->monkblock++;
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if(!IS_NPC(ch) && !IS_NPC(victim)) chance -= (char_hitroll(ch) * 0.1);
    ac = char_ac(victim)*-1;
    if(char_ac(victim) > 0) ac = 0;
    if(!IS_NPC(victim) && !IS_NPC(ch)) chance += (ac * 0.01);
...
if ( IS_CLASS(ch, CLASS_DEMON) && !IS_SET(ch->warp, WARP_QUICKNESS) )
  ...
  /* Some shit for Earth Embrace )Mage Power(, works on NPC's.. */
    ...
    if (!IS_NPC(ch) && IS_SET(ch->flag2, AFF2_BLINK_1ST_RD))
	return FALSE;
    if (!IS_NPC(ch) && IS_SET(ch->flag2, AFF2_BLINK_2ND_RD))
	return FALSE;
...
*/
    ...
    if ( IS_NPC(victim) )
    {
      if ( victim->level < 100 )
        chance = 15;
      if ( victim->level >= 100 )
        chance = ( (victim->level/10) + number_range(5,10) );
      if ( victim->level >= 500 )
        chance = 50;
      chance += config_info->bonus;
    }
...
void raw_kill( CHAR_DATA *victim, CHAR_DATA *ch )
{
  ...
  if (!IS_NPC(victim) && victim->fatal > 0) death_message(ch,victim); 
    ...
    if (number_range(1,2) == 1) {
        money = victim->level / 2;
        ch->gold += money;
        sprintf(buf,"{wYou recieve{y %d {Ygold {wcoins.{x\n\r",money);
        stc(buf,ch);
    }
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( (( get_age(gch) - 17) * 2 ) < 5 || dxp_info.dxp == 1) {
    stc("{D[{wBonus{r: ",gch);
    if ( (( get_age(gch) - 17) * 2 ) < 5 ) { stc("{Cnewbie ",gch); }
    if (dxp_info.dxp == 1 ) { stc("{Cdouble ",gch); }
    stc("{D]{x\n\r",gch); }
	xp = xp_compute( gch, victim ) / members;
	sprintf( buf, "{D[{wYou recieve{C %d {wexperience points{D]{x\n\r", xp );
   ...
   if (IS_CLASS(ch, CLASS_DEMON) || (IS_CLASS(ch, CLASS_DROW)))   
	  ...
	  if(IS_CLASS(ch, CLASS_DEMON))
	  {
            sprintf(buf,"#cYou recieve %d demon points.#n\n\r",dmpt);
            send_to_char(buf, ch);
	  }
	  else if(IS_CLASS(ch, CLASS_DROW))
	  {
            sprintf(buf,"#cYou recieve %d drow points.#n\n\r",dmpt);
            send_to_char(buf, ch);
	  }
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
  ...
  if ( dt >= TYPE_ADD_AGG ) 
	...
	while ( dt >= TYPE_ADD_AGG ) dt -= TYPE_ADD_AGG;
         ...
         if ( dam ==   0 ) { vs = "#B miss#n";  vp = "#B misses#n";     }
    else if ( dam <=  50 ) { vs = "#B lightly#n";vp = "#B lightly#n";   }
    else if ( dam <= 100 ) { vs = "";           vp = "";                }
    else if ( dam <= 250 ) { vs = "#B hard#n";  vp = "#B hard#n";               }
    else if ( dam <= 500 ) { vs = "#G very hard#n";vp = "#G very hard#n";       }
    else if ( dam <= 750 ) { vs = "#G extremely hard#n";vp = "#G extremely hard#n";}
    else if ( dam <= 1000 ){ vs = "#G incredibly hard#n";vp = "#G incredibly hard#n";}
    else if ( dam <= 1250 ){ vs = "#R unbelievably hard#n"; vp = "#R unbelievably hard#n";}
    else if ( dam <= 1500 ){ vs = "#R unpleasantly hard#n"; vp = "#R unpleasantly hard#n";}
    else if ( dam <= 1750 ){ vs = "#R ludicrously hard#n"; vp = "#R ludicrously hard#n";}
    else if ( dam <= 2000 ){ vs = "#b insanely hard#n"; vp = "#b insanely hard#n";}
    else if ( dam <= 2250 ){ vs = "#b amazingly hard#n"; vp = "#b amazingly hard#n";}
    else if ( dam <= 2500 ){ vs = "#b montrously hard#n"; vp = "#b montrously hard#n";}
    else if ( dam <= 2750 ){ vs = "#g for superior damage#n"; vp = "#g for superior damage#n";}
    else if ( dam <= 3000 ){ vs = "#g for crippling damage#n"; vp = "#g for crippling damage#n";}
    else if ( dam <= 3500 ){ vs = "#g for unspeakable damage#n"; vp = "#g for unspeakable damage#n";}
    else if ( dam <= 4000 ){ vs = "#r for ULTIMATE damage#n"; vp = "#r for ULTIMATE damage#n";}
    else                   { vs = "#r HARDER THAN GOD#n"; vp = "#r HARDER THAN GOD#n";}
...
void trip( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (IS_DEMAFF(victim,DEM_UNFOLDED))
	    return;
	if (IS_CLASS(victim, CLASS_DROW) && IS_SET(victim->pcdata->powersd[1], DPOWER_LEVITATION)) return;
    }
    if ( victim->wait == 0)
    {
...
/* Autostancing - Loki */
    ...
    ch->gifts[REDTALONS] < 4 && victim->position == POS_STANDING)
    {
    ...
    if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_NINJA) && 
    		   ch->pcdata->powerss[NPOWER_NINGENNO] > 1 
...
void do_circle( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_WEREWOLF))
	  ...
	  return;
        ...
        if (IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powerss[NPOWER_NINGENNO] < 6)
	{
	 stc("You have not learned this attack.",ch);
          return;
	}
	else if (IS_CLASS(ch, CLASS_WEREWOLF) && ch->gifts[STARGAZERS] < 4 )
	{
...
void do_backstab( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if (( IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powerss[NPOWER_NINGENNO] < 2 )
...
//        stc("Gaia prevents you from fleeing her child.\n\r",ch);
        sprintf(buf,"Gaia prevents you from fleeing %s.\n\r",victim->name);
        send_to_char(buf,ch);
        act("Gaia forces $n to keep fighting $N.\n\r",ch,NULL,victim,TO_ROOM);
        return;
      }
    }
	
    if(IS_WEREWOLF(victim) && IS_SET(victim->garou1,GAROU_CLENCHED) )
    ...
    if (IS_VAMPIRE(victim) && IS_SET(victim->newbits,NEW_COIL) && number_percent( ) > 15)
    {
    ...
    if (IS_HIGHLANDER(victim) && IS_SET(victim->newbits, STARE_DOWN) && number_percent( ) > 15)
    {
        sprintf(buf,"You remain locked in a duel to the death with %s!",victim->name);
        send_to_char(buf,ch);
        act("$n is locked in a duel to the death with $N.",ch,NULL,victim,TO_ROOM);
        return;
    }
      ...
      if ( IS_CLASS(ch, CLASS_DEMON) && IS_SET(ch->warp, WARP_STRONGLEGS))
{
    ...
    if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_VAMPIRE))
{
	dam *= (ch->pcdata->powersde[VAM_POTE]); 
}
	else if ( !IS_NPC(ch) && IS_CLASS(ch, CLASS_NINJA) &&
	ch->pcdata->powerss[NPOWER_CHIKYU] > 2 )
	dam *= 1.1;
     /*drow*/
	 else if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_DROW)) {
	if (IS_SET(victim->pcdata->powersd[1], DPOWER_TOUGHSKIN))
	dam /= 3;
	}
/*drow*/
    else if ( !IS_NPC(ch) && 
...
int dambonus( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int stance)
{
	...
	else if ( IS_STANCE(ch, STANCE_COUGAR) )
	{
	    ...
	    dam += dam * (ch->stance[STANCE_COBRA] / 80);
	else if ( IS_STANCE(ch, STANCE_BEAR) && ch->stance[STANCE_BEAR] > 100 )
	    dam += dam * (ch->stance[STANCE_BEAR] / 66);
...
void do_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_VAMPIRE) && victim->pcdata->powers[VAM_POTE] > 4)
    {
	store = victim->hit;
	victim->hit += dam;
	damage( ch, victim, dam, gsn_punch );
	victim->hit = store;
	if (number_percent() <= 90 && !IS_ARM_L(ch,LOST_ARM) && 
	...
	else if (number_percent() <= 90 && !IS_ARM_R(ch,LOST_ARM) && 
...
/* Autostancing - Loki */
	...
	if ( ch == vch )
	    continue;
        if ( vch->leader == ch) continue;
        if ( ch->leader == vch) continue;
...
void do_hurl( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->max_hit < victim->max_hit) { stc("They're quite a bit stronger than you, I don't think you should do that.\n\r",ch); return;}
...
/* war check shitty thing */
	...
	sprintf( log_buf, "%s just tried to use decap in a war.",
	    ch->name );
	log_string( log_buf );
        return;
    }

    if (is_safe(ch,victim)) can_decap = FALSE;
    if(!can_decap) 
    {
	send_to_char("You cannot decapitate that person.\n\r",ch);
	return;
    }
...
!IS_SET(victim->special,SPC_CHAMPION) && !IS_CLASS(ch, CLASS_DROW))
    ...
    flick = ch->race * (ch->race - victim->race) * 100 /2;
    if ( (( get_age(victim) - 17) * 2 ) < 5 && victim->race < 1)
    {
	sprintf(buf,"#F#r%s has #ybeen newbie#n #F#rcapped by %s#n.",victim->name,ch->name); 
	do_info(ch,buf);
	do_paradox(ch,"self");
        sprintf(buf,"%s is flicked for %d hitpoints.\n\r",ch->name,flick);
        do_info(ch,buf);
        ch->max_hit -= flick;
    ...
    if ((victim->race == 0 && ch->race >= 1)
    && (ch->race + 10 > victim->race))
    {
        if ((chao_info.chs == 1)) { sprintf(buf,"%s has evaded a paradox via chaos!\n\r",ch->name); do_info(ch,buf); } else {

    if ( ch->max_hit < flick)
    {
        send_to_char("You don't have enough hitpoints to do this. You -will- get flicked for this. \n\r" 
...
//	do_paradox(ch,"self"); 
 ...
 if (IS_POLYAFF(victim, POLY_NEWZULO))
    {
        REMOVE_BIT(victim->polyaff, POLY_NEWZULO);
 ...
 if (IS_POLYAFF(victim, POLY_ZULO))
  ...
  if (IS_SET(victim->newbits, NEW_TIDE))
	...
	adjust_hiscore( "status", victim, victim->race );
...
if (IS_SET(victim->newbits, NEW_TIDE))
 ...
 /*
  ...
  ch->pcdata->last_decap[0] = str_dup(victim->name);
    ...
    ch->exp += victim->exp / 2;
//    victim->exp /= 2;
    victim->exp = victim->exp -= (victim->exp*number_range(50,70)/100);
    if (!str_cmp(ch->pcdata->last_decap[0], victim->name))
    {
	ch->max_hit += number_range(100,victim->max_hit * 0.1);
...
*/
    if ( !IS_VAMPAFF(ch, VAM_CLAWS) && !IS_MONK(ch))
    {
...
/*
	if (!str_cmp(ch->pcdata->last_decap[0], victim->name))
  	{
	    send_to_char("Your unable to decap them again.\n\r",ch);
	    return;
	}
*/
/* war check shitty thing 
    ...
    {
        send_to_char("Not while in a war.\n\r", ch );
	sprintf( log_buf, "%s just tried to use tear in a war.",
	    ch->name );
	log_string( log_buf );
        return;
    }
...
void do_garotte(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) || !IS_SET(ch->pcdata->powersd[1], DPOWER_GAROTTE)) {
	send_to_char("Huh?\n\r", ch );
	return;}
...
void do_dark_garotte(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) || !IS_SET(ch->pcdata->powersd[1], DPOWER_GAROTTE)
	|| !IS_SET(ch->pcdata->powersd[1], DPOWER_DGAROTTE)) {
	send_to_char("Huh?\n\r", ch );
	return;}
