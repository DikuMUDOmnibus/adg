...
char *area_bit_name(int area_flags)
{
    ...
    if (area_flags & AREA_UNLINKED)   strcat(buf, " UNLINKED");
        else                                strcat(buf, " unlinked");
    return buf+1;
}
...
int get_area_flags_number(char *argument)
{
    ...
    if (!str_cmp(argument, "unlinked"))     return AREA_UNLINKED;
    return AREA_NONE;
}
...
void show_obj_values(CHAR_DATA *ch, OBJ_INDEX_DATA *obj)
{
	...
	case ITEM_MONEY:
        ...
        case ITEM_ARMOR:
            sprintf(buf, "Spell (Help oeditspell) (v3): %d\n\r", obj->value[3]);
            send_to_char(buf, ch);
            break;
    }
...
void display_resets(CHAR_DATA *ch)
{
	...
	final[0] = '\0';
        ...
        if ( pReset->arg3 != ch->in_room->vnum )        continue;
          ...
          || (!str_cmp(arg2, "obj") && is_number(arg3)))
        {
            ...
            if (!str_cmp(arg2, "mob")) {
				...
				if (!str_cmp(arg4, "inside") || !str_cmp(arg4, "in")) {
					...
					if (pReset->arg2 == WEAR_NONE) pReset->command = 'G';
...
void do_asave(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg1, "zone"))
    {
      ...
      // LxS- REMOVE_BIT(pArea->area_flags, AREA_CHANGED);
