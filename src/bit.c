...
char *wear_bit_name( int wear )
{
    ...
    if ( wear & ITEM_WEAR_FLOAT  ) strcat( buf, " float"      );
    if ( wear & ITEM_WEAR_SPECIAL) strcat( buf, " special"    );
    if ( wear & ITEM_WEAR_MEDAL  ) strcat( buf, " medal"      );
    if ( wear & ITEM_WEAR_BODYART) strcat( buf, " bodyart"    );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
int wear_name_bit( char* buf )
{
    ...
    if (!str_cmp( buf, "float"      ) ) return ITEM_WEAR_FLOAT;
    if (!str_cmp( buf, "special"    ) ) return ITEM_WEAR_SPECIAL;
    if (!str_cmp( buf, "medal"      ) ) return ITEM_WEAR_MEDAL;
    if (!str_cmp( buf, "bodyart"    ) ) return ITEM_WEAR_BODYART;
    return 0;
}
...
int wear_name_loc( char *buf )
{
    ...
    if ( !str_cmp( buf, "float" ) )     return WEAR_FLOAT;
    if ( !str_cmp( buf, "special" ) )   return WEAR_SPECIAL;
    if ( !str_cmp( buf, "medal" ) )     return WEAR_MEDAL;
    if ( !str_cmp( buf, "bodyart" ) )   return WEAR_BODYART;
    return WEAR_NONE;
}
...
char *wear_loc_name( int wearloc )
{
    ...
    case WEAR_FINGER_R: return "on the right finger";
     ...
     case WEAR_WRIST_R: return "on the right wrist";
       ...
       case WEAR_FLOAT: return "floating";
     case WEAR_SPECIAL: return "special";
       case WEAR_MEDAL: return "medal";
     case WEAR_BODYART: return "bodyart";
    }
}
