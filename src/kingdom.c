...
void load_kingdoms(void) {
	...
	king_table[i].recall = fread_number(fp);
        ...
        king_table[i].mem = fread_number(fp);
        king_table[i].exp = fread_number(fp);
	}	
...
//	for (i = 1; i <= MAX_KINGDOM; i++) {
	...
	fprintf(fp, "%d\n%d\n",king_table[i].pk, king_table[i].pd);
        ...
        fprintf(fp, "%d\n%d\n",king_table[i].mem,king_table[i].exp);
	}
...
stc("{mNum     Leader        PKs PDs Mem Experience  Whoname{x\n\r",ch);
	for (i = 1; i < MAX_KINGDOM; i++) {
        if (!str_cmp(king_table[i].name,"Not In")) continue;
	sprintf(buf, "{W%3d {D-{C+{D-{c %-12s  {w%-3d {w%-3d {w%-3d {y%-11d {w %s{x\n\r", 
        i,
	king_table[i].leader_name,
	king_table[i].pk,
	king_table[i].pd,
...
void do_kingdom(CHAR_DATA *ch, char *argument)
{
   ...
   char rankname[9];
	...
	if ( gch->pcdata->kingdom != ch->pcdata->kingdom) 
	    continue;
	if (!str_cmp(king_table[ch->pcdata->kingdom].leader_name,gch->name)) 
	...
	sprintf(rankname, "Prince");
	 ...
	 sprintf(rankname,"Member");
	    sprintf( buf,
	    "#2[#n%-12s#2] [#n%6ld#2] [#n%7ld#2] [#n%6ld#2] [#n%6ld#2] [#n   %-2ld   #2] [ #n%-6s #2]#n\n\r",
		gch->name ,
