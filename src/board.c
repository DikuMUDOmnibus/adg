...
{
//{ "Changes",	"Changes to ADG",		0,10,"all",DEF_INCLUDE,60,NULL,FALSE},
{ "General",	"General Discussion",				0,2,"all",DEF_INCLUDE,60,NULL,FALSE },
{ "News",	"What's New on ADG",		0,7,"all",DEF_INCLUDE,60,NULL,FALSE },
{ "Ideas",	"Suggestion for Improvement",			0,2,"all",DEF_NORMAL,60,NULL,FALSE  },
{ "Bugs",	"Typos, Bugs, Errors, Etc",			0,2,"imm",DEF_INCLUDE,60,NULL,FALSE },
{ "Personal",	"Personal Messages",				0,2,"all",DEF_EXCLUDE,60,NULL,FALSE },
{ "Immortal",	"Immortal Board",				7,7,"imm",DEF_INCLUDE,60,NULL,FALSE },
{ "Coding",	"Coders Board",					10,10,"imm",DEF_INCLUDE,60,NULL,FALSE},

};

/* The prompt that the character is given after finishing a note with ~ or END */
const char * szFinishPrompt = "(" BOLD "C" WHITE ")ontinue, (" BOLD "V" WHITE ")iew, (" BOLD "P" WHITE ")ost or (" BOLD "F" WHITE ")orget it?";
...
static void show_note_to_char (CHAR_DATA *ch, NOTE_DATA *note, int num)
{
	...
	/* Ugly colors ? */	
	sprintf (buf,
			 "[" BOLD "%4d" WHITE "] " BOLD YELLOW "%s" WHITE": " GREEN "%s" WHITE "\n\r"
	         BOLD YELLOW "Date" WHITE ":  %s\n\r"
			 BOLD YELLOW "To" WHITE ":    %s\n\r"
	         GREEN"===========================================================================" WHITE "\n\r"
	         "%s\n\r",
...
{
	if (!str_cmp (ch->name, note->sender) || IS_IMMORTAL(ch))
		return TRUE;
	...
	if (is_full_name (ch->name, note->to_list) || IS_IMMORTAL(ch))
		return TRUE;
...
if (ch->fight_timer >0) {
	...
	act (BOLD GREEN "$n starts writing a note." WHITE , ch, NULL, NULL, TO_ROOM);
	
	/* Begin writing the note ! */
	sprintf (buf, "You are now %s a new note on the " BOLD "%s" WHITE " board.\n\r"
	...
	sprintf (buf, BOLD YELLOW "From" WHITE ":    %s\n\r\n\r", ch->name);
	send_to_char (buf,ch);
	...
	if (!ch->pcdata->in_progress->text) /* Are we continuing an old note or not? */
	{
		...
		{
		case DEF_NORMAL:
			sprintf (buf, "If you press Return, default recipient \"" BOLD "%s" WHITE "\" will be chosen.\n\r",
					  ch->pcdata->board->names);
			break;
		case DEF_INCLUDE:
			sprintf (buf, "The recipient list MUST include \"" BOLD "%s" WHITE "\". If not, it will be added automatically.\n\r",
						   ch->pcdata->board->names);
			break;
	
		case DEF_EXCLUDE:
			sprintf (buf, "The recipient of this note must NOT include: \"" BOLD "%s" WHITE "\".",
						   ch->pcdata->board->names);
	...
	{
		sprintf (buf, BOLD YELLOW "To" WHITE ":      %s\n\r"
		              BOLD YELLOW "Expires" WHITE ": %s\n\r"
		              BOLD YELLOW "Subject" WHITE ": %s\n\r", 
		...
		send_to_char ("\n\rEnter text. Type " BOLD "~" WHITE " or " BOLD "END" WHITE " on an empty line to end note.\n\r"
		                    "=======================================================\n\r",ch);
...
static void do_nread (CHAR_DATA * ch, char *argument)
{
   ...
   {        
      ...
      if (next_board (ch))
         sprintf (buf,
                  "Changed to next board, %s, where there are %d notes.\n\r",
                  ch->pcdata->board->short_name, total_notes (ch,&boards[board_number(ch->pcdata->board)]));
...
/* Changed Toaster 15/03/00 so i can remove everybodys notes */
	if (!str_cmp(ch->name,p->sender) && !IS_IMMORTAL(ch) )
	{
...
static void do_nlist (CHAR_DATA *ch, char *argument)
{
	...
	send_to_char (BOLD "Notes on this board:" WHITE "\n\r"
	              RED "Num> Author        Subject" WHITE "\n\r",ch);
	...
	for (p = ch->pcdata->board->note_first; p; p = p->next)
	{
		...
		if (is_note_to(ch,p))
		{
			...
			{
				sprintf (buf, BOLD "%3d" WHITE ">" BLUE BOLD "%c" WHITE YELLOW BOLD "%-13s" WHITE YELLOW " %s" WHITE " \n\r",
				               num, 
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
		send_to_char("\n\r", ch);
		send_to_char (BOLD RED "Num         Name Unread Description" WHITE "\n\r"
		              RED "=== ============ ====== ===========" WHITE "\n\r",ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
			...
			{ /* watch out for the non-portable &%c !!! */
				sprintf (buf, BOLD "%2d" WHITE "> " GREEN BOLD "%12s" WHITE " [%4d" WHITE "] " YELLOW "%s" WHITE "\n\r", 
		...
		if (ch->pcdata->board != NULL) {
		sprintf (buf, "\n\rYour current board is " BOLD "%s" WHITE ".\n\r", ch->pcdata->board->short_name);
		send_to_char (buf,ch);
	...
	if (is_number(argument))
	{
		...
		if (count == number) /* found the board.. change to it */
		{
			...
			sprintf (buf, "Current board changed to " BOLD "%s" WHITE ". %s.\n\r",boards[i].short_name,
	...
	switch (ch->pcdata->board->force_type)
	{
		case DEF_NORMAL: /* default field */
			if (!buf[0]) /* empty string? */
			{
				...
				sprintf (buf, "Assumed default recipient: " BOLD "%s" WHITE "\n\r", ch->pcdata->board->names);
		...
		case DEF_EXCLUDE: /* forced exclude */
			if (is_full_name (ch->pcdata->board->names, buf))
			{
				sprintf (buf, "You are not allowed to send notes to %s on this board. Try again.\n\r"
				         BOLD YELLOW "To" WHITE ":      ", ch->pcdata->board->names);
				write_to_buffer (d, buf, 0);
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!buf[0])		
	{
		...
		write_to_buffer (d, BOLD YELLOW "Subject" WHITE ": ", 0);
	}
	...
	/* advance to next stage */
	{
		...
		if (IS_IMMORTAL(ch)) /* immortals get to choose number of expire days */
		{
			sprintf (buf,"\n\rHow many days do you want this note to expire in?\n\r"
			             "Press Enter for default value for this board, " BOLD "%d" WHITE " days.\n\r"
           				 BOLD YELLOW "Expire" WHITE ":  ",
...
void handle_con_note_expire(DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!buf[0]) /* assume default expire */
		...
		if (!is_number(buf))
		{
			...
			write_to_buffer (d,BOLD YELLOW "Expire" WHITE ":  ",0);
			return;
		}
		else
		{
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			...
			case 'v': /* view note so far */
				if (ch->pcdata->in_progress->text)
				{
					write_to_buffer (d,GREEN "Text of your note so far:" WHITE "\n\r",0);
			...
			case 'p': /* post note */
				...
				{
/*Changed by Toaster to add colour*/
				    sprintf(buf2, "%s#w posted a note entitled: %s.\n\r", 
					ch->name,
					ch->pcdata->in_progress->subject);
				    do_info(ch, buf2);
				}
				...
				ch->pcdata->in_progress = NULL;
				act (BOLD GREEN "$n finishes $s note." WHITE , ch, NULL, NULL, TO_ROOM);
				break;
