...
// Created  by Nightstar to aid reclassing. 22/03/00
void do_alert( CHAR_DATA *ch, char *argument )
{
	send_to_char("#rThe Drow have been removed from this world, Please speak to an immortal about being reclassed.#n\n\r", ch);
	sprintf( log_buf, " #B[#GALERT#B]#n %s has tried to use a drow command, please reclass.", ch->name );
	return;
}
...
stc("{m---------------------------------------------------------------------------{x\n\r",ch);
stc("{W Syntax{D:{w Grant <person> <power>{x\n\r",ch);
stc("{m---------------------------------------------------------------------------{x\n\r",ch);
stc("{M    Drowfire     {D[{w 2500{D]{M Darkness   {D[{w 7500{D]{M Drowshield {D[{w 5000{D]{x\n\r",ch);
stc("{M    Drowpoison   {D[{w 2500{D]{M Drowsight  {D[{w 5000{D]{M Levitation {D[{w 1000{D]{x\n\r",ch);
stc("{M    Shadowwalk   {D[{w10000{D]{M Garotte    {D[{w 5000{D]{M Spiderarms {D[{w 7500{D]{x\n\r",ch);
stc("{M    Drowhate     {D[{w10000{D]{M Spiderform {D[{w15000{D]{M Web        {D[{w 5000{D]{x\n\r",ch);
stc("{M    Dgarotte     {D[{w 2500{D]{M Confuse    {D[{w 2500{D]{M Glamour    {D[{w 5000{D]{x\n\r",ch);
stc("{M    Earthshatter {D[{w 7500{D]{M Speed      {D[{w 7500{D]{M Toughskin  {D[{w 7500{D]{x\n\r",ch);
stc("{M    LlothsRegen  {D[{w10000{D]{x\n\r",ch);
stc("{m---------------------------------------------------------------------------{x\n\r",ch);
stc("{W Professions{D:{w Warrior, Mage, Cleric{x\n\r",ch);
stc("{m---------------------------------------------------------------------------{x\n\r",ch);
...
/*
  ...
  {		send_to_char( "They already have a profession.\n\r", ch );
    ...
    if (IS_SET(victim->pcdata->powersd[1], inpart))
    {
    ...
    SET_BIT(victim->pcdata->powersd[1], inpart);
	 ...
	 if (IS_CLASS(victim, CLASS_DROW))
{
  ...
  if(( victim->max_hit < 3000) && (victim->max_move < 3000) &&
        (victim->max_mana < 3000))
        {
        send_to_char("They do not have 3000 hit/mana/move", ch);
        return;
        }
...
void do_drowfire( CHAR_DATA *ch, char *argument ) {
	...
	if(!IS_CLASS(ch, CLASS_DROW) ||
!IS_SET(ch->pcdata->powersd[1],DPOWER_DROWFIRE)) {
		send_to_char("Huh?\n\r", ch );
		return;}
...
!IS_SET(ch->pcdata->powersd[1],DPOWER_DROWFIRE)) {
    ...
    stc("You chant some arcane words on them.\n\r\n\rThey are engulfed in drowfire!\n\r",ch);
    sprintf(buf,"%s chants some arcane words on you.\n\rYou are engulfed in drowfire!\n\r",ch->name);
    stc(buf,victim);
        return;
}
...
void do_shadowwalk(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1],DPOWER_SHADOWWALK)) {
	send_to_char("Huh?\n\r", ch );
	return;}
...
/* Changed Toaster 05/03/00 to allow drow to shadowwalk anywhere
	if (!IS_SET(victim->in_room->room_flags, ROOM_DARK) && 
	!IS_SET(victim->in_room->room_flags, ROOM_TOTAL_DARKNESS)) {
	send_to_char("You failed.\n\r", ch );
	return;}

Players are going to get lost when travelling using powers sometimes

	if( (number_percent() < 15 )
	{
	act("You walk into the shadows but become lost.", ch, NULL, NULL, TO_CHAR);
...
void do_drowhate(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1], DPOWER_DROWHATE))
	{send_to_char("Huh?\n\r", ch );
	return;}
...
void do_spiderform( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1],
DPOWER_SPIDERFORM))
	{send_to_char("Huh?\n\r", ch );
	return;}
...
/*Changed by Toaster to 300 like alt3 
        ch->damroll -= 50;
        ch->hitroll -= 50; 
*/
	ch->damroll -= 300;
...
/*Changed by Toaster 27/01/00 to be like AR3's spiderform. was 50*/
	ch->damroll += 300;
	ch->hitroll += 300;
        return;
    }
...
void do_spiderform( CHAR_DATA *ch, char *argument ) 
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1], DPOWER_SPIDERFORM))
	{send_to_char("Huh?\n\r", ch );
	return;}
...
/* Changed Toaster so immortals not show on drows 06/03/00*/
...
void do_drowsight(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1],DPOWER_DROWSIGHT)) {
	send_to_char("Huh?\n\r", ch );
	return;}
...
void do_drowpowers(CHAR_DATA *ch,char *argument)
{
	...
	if (IS_SET(ch->pcdata->powersd[1], DPOWER_SPEED))
	send_to_char("You move quite quickly.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_TOUGHSKIN))
	send_to_char("Your skin has been toughened.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_DROWFIRE))
	send_to_char("You can call Drowfire upon your enemies.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_DARKNESS))
	send_to_char("You can summon a globe of darkness.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_DROWSIGHT))
	send_to_char("You can use enhanced sight.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_LEVITATION))
	send_to_char("You can levitate in the air.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_DROWSHIELD))
	send_to_char("You can shield your aura.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_DROWPOISON))
	send_to_char("You have control over deadly poisons.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_SHADOWWALK))
	send_to_char("You can walk through the shadows.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_GAROTTE)) 
	send_to_char("You have mastered the art of garotte.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_DGAROTTE))
	...
	if (IS_SET(ch->pcdata->powersd[1], DPOWER_SPIDERFORM))
	send_to_char("You can mutate into a large spider.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_WEB))
	send_to_char("You can shoot a spidery web at opponents.\n\r", ch);

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_GLAMOUR))
	send_to_char("You can glamourosly change the appearance of items.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_CONFUSE))
	send_to_char("You can confuse your enemies during battle.\n\r", ch );

	if (IS_SET(ch->pcdata->powersd[1], DPOWER_EARTHSHATTER))
...
void do_darkness(CHAR_DATA *ch, char *argument) 
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1], DPOWER_DARKNESS)) {
/* Changed by Toaster 04/02/00 becuase this next part was commented out
trying to fix drow darkness*/
...
!str_cmp(arg2, "NONAME") || !str_cmp(arg2, "NONAME"))
	...
	if (!IS_CLASS(ch,CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1],DPOWER_GLAMOUR)) {
	send_to_char("Huh?\n\r", ch );
	return;}
...
/* Changed Toaster 06/03/00 so you have to claim an item to glamour*/
 if (!IS_IMMORTAL(ch) && (obj->questowner == NULL || str_cmp(ch->name,obj->questowner)))
    {
        send_to_char("You can only use glamour on an item you own.\n\r", ch);
        return;
    }
...
void do_dconfuse(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_SET(ch->pcdata->powersd[1], DPOWER_CONFUSE))
    {
        send_to_char("You have not been gifted the ability to confuse.\n\r", ch);
        return;
    }
...
void do_earthshatter( CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_SET(ch->pcdata->powersd[1], DPOWER_EARTHSHATTER)) {
	send_to_char("Huh?\n\r", ch );
	return;}
